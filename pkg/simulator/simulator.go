...
var smsFilename = "sms.data"
var voiceFilename = "voice.data"
var emailFilename = "email.data"
var billingFilename = "billing.data"
...
func shuffleSmsData() {
	...
	err := ioutil.WriteFile(getFilapathByFilename(viper.GetString("data.path")+smsFilename), []byte(data), 0644)
	if err != nil {
...
func shuffleVoiceData() {
	...
	err := ioutil.WriteFile(getFilapathByFilename(viper.GetString("data.path")+voiceFilename), []byte(data), 0644)
	if err != nil {
		fmt.Printf("Error in write sms data: %s", err.Error())
	}
}
...
func shuffleEmailData() {
	...
	err := ioutil.WriteFile(getFilapathByFilename(viper.GetString("data.path")+emailFilename), []byte(data), 0644)
	if err != nil {
